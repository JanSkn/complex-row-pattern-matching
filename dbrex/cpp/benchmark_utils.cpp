...
duration<double> BenchmarkUtils::benchmarkSelfJoins(const size_t& testNum, const size_t& twNum) {
    ...
    vector<vector<vector<string>>> selfJoins = {   
        ...
        {{"SELECT M.*, T.*, C.* "
        "FROM postgres.public.crimedatabenchmark M "
        "JOIN postgres.public.crimedatabenchmark T ON T.category = 'Theft in / from a motor vehicle' AND M.id < T.id "
        "AND T.id BETWEEN M.id AND M.id + 49 "
        "JOIN postgres.public.crimedatabenchmark C ON C.category = 'Confirmed Theft' AND T.id < C.id "
        ...
        "JOIN postgres.public.crimedatabenchmark C ON C.category = 'Confirmed Theft' AND T.id < C.id "
        "AND C.id BETWEEN M.id AND M.id + 49 "
        "JOIN postgres.public.crimedatabenchmark H ON H.category = 'Home Invasion' AND C.id < H.id "
        ...
        {{"SELECT M.*, C.*, H.* "
        "FROM postgres.public.crimedatabenchmark M "
        "JOIN postgres.public.crimedatabenchmark C ON C.category = 'Confirmed Theft' AND M.id < C.id "
        "AND C.id BETWEEN M.id AND M.id + 49 "
        "JOIN postgres.public.crimedatabenchmark H ON H.category = 'Home Invasion' AND C.id < H.id "
        "AND H.id BETWEEN M.id AND M.id + 49 "
        "WHERE M.category = 'Mischief'",
        ...
        "JOIN postgres.public.crimedatabenchmark C ON C.category = 'Confirmed Theft' AND M.id < C.id "
        "AND C.id BETWEEN M.id AND M.id + 49 "
        "JOIN postgres.public.crimedatabenchmark V ON V.category = 'Motor vehicle theft' AND C.id < V.id "
